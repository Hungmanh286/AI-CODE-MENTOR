...
def test_chunk_completeness():
    ...
    if sections_count < expected_chunks * 0.5:  # At least 50% of expected
        logger.info(f"⚠️  WARNING: Fewer sections than expected")
        logger.info(f"   Expected ~{expected_chunks}, got {sections_count}")
    else:
        logger.info(f"✅ PASSED: All chunks processed")
...
def test_sequential_vs_parallel_accuracy():
    ...
    if seq_len > 0:
        ...
        if diff_percent > 50:
            ...
            logger.info("   This is expected as parallel uses vision model (different parser)")
        else:
...
def test_error_handling():
    ...
    try:
        ...
        logger.info(f"✅ PASSED: Error handled correctly")
