...
class ParallelConfig:
    MAX_WORKERS: int = 10  # Số lượng threads xử lý đồng thời
    RPM_LIMIT: int = 15    # Requests per minute (phụ thuộc vào plan của bạn)
...
def process_single_chunk(chunk_data: Tuple[int, List[str]], use_gemini: bool = True) -> Tuple[int, str]:
    """
    ...
    for attempt in range(ParallelConfig.RETRY_ATTEMPTS):
        try:
            ...
            if attempt < ParallelConfig.RETRY_ATTEMPTS - 1:
                print(f"⚠️  Chunk {chunk_index} failed (attempt {attempt + 1}/{ParallelConfig.RETRY_ATTEMPTS}): {str(e)}")
                time.sleep(ParallelConfig.RETRY_DELAY)
            else:
                print(f"❌ Chunk {chunk_index} failed after {ParallelConfig.RETRY_ATTEMPTS} attempts: {str(e)}")
...
def parse_pdf_parallel(
    ...
    chunk_size = ParallelConfig.CHUNK_SIZE_SMALL if total_pages < 50 else ParallelConfig.CHUNK_SIZE_LARGE
    ...
    print(f"📈 Average time per chunk: {duration.total_seconds() / total_chunks:.2f} seconds")
...
if __name__ == "__main__":
    ...
    # Save kết quả
    output_file = f"parsed_output_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    with open(output_file, "w", encoding="utf-8") as f:
        f.write(doc)
    print(f"\n💾 Output saved to: {output_file}")
    
    # Embedding vào vector store
    print("\n🔍 Embedding document into vector store...")
    embedding_document([doc], "test_session_parallel")
    print("✅ Done!")
